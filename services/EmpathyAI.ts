...
export class EmpathyAI {
  private userName: string;
  private currentModel: string;
  private history: ConversationHistory;
  private safetyFilter: SafetyFilter;
  private _lastStarter: number | null = null;
  private _lastClosing: string | null = null;
  ...
  private readonly EMPATHY_STARTERS = [
  ...
  constructor(userName: string = 'User', defaultModel: string = 'empathetic') {
  ...
  greet(): string {
    const greetings = [
      `Hello ${this.userName}! I'm JustinBot, your empathetic AI companion. I'm here to listen, support, and help you navigate whatever is on your mind. How are you feeling today?`,
      `Hi ${this.userName}! I'm here for you. Whether you need to talk, need advice, or just want someone to listen, I'm ready. What's on your mind?`,
      `Welcome ${this.userName}! I'm JustinBot, and I'm here to provide compassionate support. How can I help you today?`
    ];
  ...
  respond(userMessage: string): string {
    const result = this.processMessage(userMessage);
   ...
   */
  selectBestModel(message: string): string {
   ...
   */
  private processMessage(userMessage: string): ProcessMessageResult {
    ...
    const [severity, isFlagged, reason] = this.safetyFilter.analyzeMessage(userMessage);

    // Store in history
    this.history.addMessage('user', userMessage, reason || null);

    // Handle flagged content
    if (isFlagged && ['critical', 'warning'].includes(severity)) {
   ...
   */
  private _findSimilarTopic(message: string): any[] {
   ...
   */
  private _generateEmpatheticResponse(message: string, model: string, similarHistory: any[]): string {
    let response = '';
   ...
   */
  private _modelSpecificResponse(message: string, modelInfo: ModelInfo): string {
   ...
   */
  private _getSupportiveClosing(): string {
    const closings = [
      "I'm here for you.",
